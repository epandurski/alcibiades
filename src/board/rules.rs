...
///
/// **Note:** The initial move scores for the moves generated by
/// `generate_moves` method will be:
///
...
impl<T: BoardEvaluator + 'static> Position<T> {
    ...
    fn from_fen(fen: &str) -> Result<Position<T>, String> {
        ...
        let board = try!(Board::from_raw_parts(placement, to_move, castling, en_passant_square)
                             .map_err(|_| fen));
        Ok(Position {
    ...
    #[inline(always)]
    unsafe fn board_mut(&self) -> &mut Board<T> {
        &mut *self.board.get()
    }
}
