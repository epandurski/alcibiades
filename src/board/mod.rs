...
pub trait MoveGenerator: Sized + Send + Clone + SetOption {
    ...
    ///
    /// The moves are pushed to `moves`. All generated moves with
    ...
    ///
    /// The moves are pushed to `moves`. This method always generates
    /// a **subset** of the moves generated by `generate_all`.
    ///
    /// All legal captures and, when in check, all legal check
    /// evasions will be included. Optionally, other moves might be
    /// included too. For example: moves that give check, or pawn
    /// promotions to queen. This is done carefully though, to avoid
    /// creating an infinitely large search tree. In order to be able
    /// to decide whether to include other moves or not,
    ...
    ///   of play gave check (<= `ply`).
    fn generate_forcing<T: PushMove>(&self, ply: u8, checks: u8, moves: &mut T);
