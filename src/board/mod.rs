...
pub trait MoveGenerator: Sized + Send + Clone + SetOption {
    ...
    ///
    /// The generated moves are pushed to `move_stack`.
    ///
    /// **Note:** A pseudo-legal move is a move that is otherwise
    /// legal, except it might leave the king in check.
    fn generate_all<T: PushMove>(&self, move_stack: &mut T);

    /// Generates moves for quiescence search, possibly including some
    /// pseudo-legal moves too.
    ///
    /// The generated moves are pushed to `move_stack`. All legal
    /// check evasions and captures will be included. Optionally,
    /// other moves might be included too. For example: moves that
    /// give check, or pawn promotions to queen. This is done
    /// carefully though, to avoid creating an infinitely large search
    /// tree. In order to be able to decide whether to include other
    /// moves, `generate_focing` should be supplied with some
    /// information about the currently running quiescence search:
    ///
    /// * `ply` specifies the number of moves in the current line of
    ///   play (half-moves).
    /// * `checks` specifies how many of the moves in the current line
    ///   of play gave check.
    fn generate_forcing<T: PushMove>(&self, ply: u8, checks: u8, move_stack: &mut T);
