...
pub type AttackSets = [[u64; 64]; 5];  // for example
                                   // attack_set[Pieces::Queen][0]
                                   // gives the attack set for a queen
                                   // at A1.


pub fn generate_attack_sets() -> AttackSets {

    fn square2grid_index(i: usize) -> usize {
        ((i / 8) * 10 + (i % 8) + 21) as usize
    }
    ...
    // At the end will hold the attack bitset for each piece on each
    // possible square.
    let mut attack_sets = [[0u64; 64]; 5];
    ...
    for piece_type in 0..5 {
        for square in 0..64 {
            ...
            attack_sets[piece_type][square] = bitset;
        }
    }
    attack_sets
}
