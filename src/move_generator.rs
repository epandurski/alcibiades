...
pub trait MoveGenerator: Clone + SetOption + Send + 'static {
    ...
    /// the king.
    ///
    /// **Important note:** The calculated bitboard is temporarily
    /// stored in move generator's instance. In case another call to
    /// `checkers` is made before any move was done/undone, `checkers`
    /// will return the stored bitboard instead of re-calculating it,
    /// thus saving time.
