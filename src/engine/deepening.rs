...
pub struct Deepening<T: SearchExecutor<ReportData = Vec<Move>>> {
    tt: Arc<T::HashTable>,
...
impl<T: SearchExecutor<ReportData = Vec<Move>>> Deepening<T> {
...
impl<T: SearchExecutor<ReportData = Vec<Move>>> SetOption for Deepening<T> {
...
impl<T: SearchExecutor<ReportData = Vec<Move>>> SearchExecutor for Deepening<T> {
...
/// move. This mode is very useful for chess analysis, but can
/// sometimes make the search slower.
///
/// # Usage
///
