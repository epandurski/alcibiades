...
impl Position {
    ...
    /// check or the position is a draw due to repetition or the
    /// 50-moves rule.
    #[inline]
    ...
    /// position is a draw due to repetition or the 50-moves rule.
    ///
    /// **Important note:** Repeating positions are considered final
    ...
    pub fn generate_moves(&self, move_sink: &mut MoveSink) {
        if !self.state().is_repeated && self.state().halfmove_clock < 100 {
    ...
    unsafe fn do_move_unsafe(&self, m: Move) -> bool {
        ...
        if (state.is_repeated || state.halfmove_clock >= 100) && Board::is_null_move(m) {
            return false;
        }
        let board = self.board_mut();
