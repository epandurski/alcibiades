...
impl Board {
    ...
    pub fn try_move_digest(&self, move_digest: MoveDigest) -> Option<Move> {
        let move_type = extract_move_type(move_digest);
        let orig_square = extract_orig_square(move_digest);
        let dest_square = extract_dest_square(move_digest);
        let king_square = self.king_square();
        let checkers = self.checkers();
        ...
        if piece == PAWN {
            ...
            match dest_square_bb {
                ...
                x if x & BB_PAWN_PROMOTION_RANKS != 0 => {
                    ...
                    promoted_piece_code = extract_aux_data(move_digest);
                }
                _ => {
