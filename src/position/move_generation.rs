...
impl Board {
    ...
    fn is_legal(&self) -> bool {
        ...
        {
            debug_assert_eq!(self._occupied, occupied);
            debug_assert!(self._checkers.get() == BB_UNIVERSAL_SET ||
                    self._checkers.get() == self.attacks_to(them, bitscan_1bit(our_king_bb)));
            debug_assert!(self._pinned.get() == BB_UNIVERSAL_SET ||
                    self._pinned.get() == self.find_pinned());
            debug_assert!(self._king_square.get() > 63 ||
